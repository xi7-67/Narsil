<script lang="ts">
	...
	type ShakaPlayerInstance = {
		...
		getNetworkingEngine?: () => {
			registerRequestFilter: (
				callback: (
					type: unknown,
					request: { method: string; uris: string[] },
				) => void,
			) => void;
		};
	};
	...
	function cacheFlacFallback(
		trackId: number,
		result: DashManifestResult | DashManifestWithMetadata,
	) {
	...
	async function resolveStream(
		...
		if (isHiResQuality(quality)) {
			throw new Error(
				"Attempted to resolve hi-res stream via standard resolver",
			);
		}
	...
	function pruneStreamCache() {
		...
		const keepKeys = new Set<string>();
		const baseQualities: AudioQuality[] = isHiResQuality(quality)
			? ["LOSSLESS"]
			: [quality];
		const current = $playerStore.currentTrack;
		if (current && !isSonglinkTrack(current)) {
	...
	async function preloadDashManifest(track: Track) {
		...
		if (
			dashManifestCache.has(cacheKey) ||
		...
		preloadingCacheKey = cacheKey;
		try {
			const result = await losslessAPI.getDashManifestWithMetadata(
				track.id,
				"HI_RES_LOSSLESS",
			);
	 ...
	 */
	async function convertSonglinkTrackToTidal(
		...
		if (songlinkTrack.tidalId) {
			try {
				const trackLookup = await losslessAPI.getTrack(
					songlinkTrack.tidalId,
				);
				...
				console.warn(
					"Failed to fetch track using pre-calculated tidalId, falling back to extraction",
					e,
				);
			}
		}
		...
		if (!tidalInfo || tidalInfo.type !== "track") {
			...
			console.warn(
				"No TIDAL track in Songlink data, attempting conversion...",
			);
			const fallbackTidalInfo = await convertToTidal(
				songlinkTrack.sourceUrl,
				{
					userCountry: "US",
					songIfSingle: true,
				},
			...
			if (!fallbackTidalInfo || fallbackTidalInfo.type !== "track") {
				throw new Error(
					`Could not find TIDAL equivalent for: ${songlinkTrack.title}`,
				);
			}
			...
			if (!trackLookup?.track) {
				throw new Error(
					`Failed to fetch TIDAL track for: ${songlinkTrack.title}`,
				);
			}
		...
		if (!Number.isFinite(trackId) || trackId <= 0) {
			...
			console.warn(
				`Non-numeric TIDAL ID (${tidalInfo.id}), attempting fallback conversion...`,
			);
			const fallbackTidalInfo = await convertToTidal(
				songlinkTrack.sourceUrl,
				{
			...
			if (!Number.isFinite(fallbackId) || fallbackId <= 0) {
				throw new Error(
					`No valid TIDAL track found for: ${songlinkTrack.title}`,
				);
			}
		...
		console.log(
			"Successfully converted to TIDAL track:",
			trackLookup.track.title,
		);
		return trackLookup.track;
	}
	...
	$effect(() => {
		...
		if (current && isSonglinkTrack(current)) {
			...
			if (convertingTracks.has(current.id)) {
				console.log(
					"[Conversion Effect] Track already being converted, skipping",
				);
				return;
			}
			...
			console.log(
				"[Conversion Effect] Starting conversion for:",
				current.title,
			);
			...
			convertSonglinkTrackToTidal(current)
				.then((tidalTrack) => {
					console.log(
						"[Conversion Effect] Conversion SUCCESS:",
						tidalTrack.title,
						"TIDAL ID:",
						tidalTrack.id,
					);
					...
					if (
						...
						console.log(
							"[Conversion Effect] Updating player with converted track",
						);
						playerStore.setTrack(tidalTrack);
					} else {
						console.log(
							"[Conversion Effect] Track changed during conversion, not updating",
						);
					}
				})
				.catch((error) => {
					console.error(
						"[Conversion Effect] Conversion FAILED:",
						error,
					);
					alert(
				...
				.finally(() => {
					...
					console.log(
						"[Conversion Effect] Finished conversion attempt for:",
						current.title,
					);
				});
		}
	...
	async function loadStandardTrack(
		track: Track,
		...
		dashPlaybackActive = false;
		const { url, replayGain, sampleRate, bitDepth } = await resolveStream(
			track,
			quality,
		);
	...
	async function loadDashTrack(
		...
		if (!cached) {
			cached = await losslessAPI.getDashManifestWithMetadata(
				track.id,
				quality,
			);
	...
	async function loadTrack(track: PlayableTrack) {
		...
		if (isSonglinkTrack(track)) {
			console.error(
				"Attempted to load SonglinkTrack directly - this should not happen!",
				track,
			);
			return;
		}
		...
		if (
			...
			trackBestQuality &&
			!isHiResQuality(trackBestQuality)
		) {
			requestedQuality = trackBestQuality;
		}

		if (
			dashFallbackAttemptedTrackId &&
			dashFallbackAttemptedTrackId !== tidalTrack.id
		) {
			dashFallbackAttemptedTrackId = null;
		}
		...
		try {
			if (isHiResQuality(requestedQuality)) {
				try {
					...
					const dashResult = await loadDashTrack(
						tidalTrack,
						hiResQuality,
						sequence,
					);
					...
					console.info(
						"Dash endpoint returned FLAC fallback. Using lossless stream.",
					);
				} catch (dashError) {
					const coded = dashError as { code?: string };
					if (coded?.code === DASH_MANIFEST_UNAVAILABLE_CODE) {
						dashManifestCache.delete(
							getCacheKey(tidalTrack.id, "HI_RES_LOSSLESS"),
						);
					}
					console.warn(
						"DASH playback failed, falling back to lossless stream.",
						dashError,
					);
				}
			...
			if (
				...
				try {
					...
					console.error(
						"Secondary lossless fallback failed:",
						fallbackError,
					);
				}
			}
	...
	function handleTimeUpdate() {
		if (audioElement) {
			...
			const remaining =
				($playerStore.duration ?? 0) - audioElement.currentTime;
	...
	async function fallbackToLosslessAfterDashError(reason: string) {
		...
		const sequence = ++loadSequence;
		console.warn(
			`Attempting lossless fallback after DASH playback error (${reason}).`,
		);
		try {
			dashPlaybackActive = false;
			playerStore.setLoading(true);
	...
	function handleLoadedData() {
		...
		if (
			audioElement &&
			state.currentTime > 0 &&
			Math.abs(audioElement.currentTime - state.currentTime) > 1
		) {
	...
	function handlePrevious() {
		if (
			audioElement &&
			(audioElement.currentTime > 5 || $playerStore.queueIndex <= 0)
		) {
			...
			await losslessAPI.downloadTrack(track.id, quality, filename, {
				...
				onProgress: (progress: TrackDownloadProgress) => {
					if (progress.stage === "downloading") {
						...
						downloadUiStore.updateTrackStage(
							taskId,
							progress.progress,
						);
					}
				...
				onFfmpegCountdown: ({ totalBytes }) => {
					if (typeof totalBytes === "number") {
						downloadUiStore.startFfmpegCountdown(totalBytes, {
							autoTriggered: false,
						});
					} else {
						downloadUiStore.startFfmpegCountdown(0, {
							autoTriggered: false,
						});
					}
				},
				onFfmpegStart: () => downloadUiStore.startFfmpegLoading(),
				onFfmpegProgress: (value) =>
					downloadUiStore.updateFfmpegProgress(value),
	...
	function formatSampleRate(value?: number | null): string | null {
		...
		const precision =
			kilohertz >= 100 ||
			Math.abs(kilohertz - Math.round(kilohertz)) < 0.05
				? 0
				: 1;
	...
	function updateMediaSessionPositionState() {
		...
		const duration = Number.isFinite(durationFromAudio)
			? durationFromAudio
			: storeState.duration;
	...
	function registerMediaSessionHandlers() {
		...
		const safeSetActionHandler = (
			...
			try {
				...
				console.debug(
					`Media Session action ${action} unsupported`,
					error,
				);
			}
		};
	...
	>
		<div class="relative mx-auto w-full max-w-screen-2xl">
			{#if $ffmpegBanner.phase !== "idle" || $activeTrackDownloads.length > 0}
				...
				>
					<div
						class="mx-auto flex w-full max-w-2xl flex-col gap-2 px-4"
					>
						{#if $ffmpegBanner.phase !== "idle"}
							...
							>
								<div class="flex items-start gap-3">
									<div class="min-w-0 flex-1">
										...
										{#if $ffmpegBanner.phase === "countdown"}
											<p
												class="mt-1 text-xs text-blue-100/80"
											>
												Starting in {$ffmpegBanner.countdownSeconds}
												seconds…
											</p>
										{:else if $ffmpegBanner.phase === "loading"}
											<p
												class="mt-1 text-xs text-blue-100/80"
											>
												Preparing encoder… {formatPercent(
													$ffmpegBanner.progress,
												)}
											</p>
								...
								{#if $ffmpegBanner.phase === "loading"}
									<div
										class="mt-3 h-1.5 overflow-hidden rounded-full bg-blue-500/20"
									>
										<div
						...
						{#each $activeTrackDownloads as task (task.id)}
							...
							>
								<div class="flex items-start gap-3">
									...
									>
										<p
											class="flex items-center gap-2 text-sm font-semibold text-gray-50"
										>
									...
									<button
										onclick={() =>
											task.cancellable
												? downloadUiStore.cancelTrackDownload(
														task.id,
													)
												: downloadUiStore.dismissTrackTask(
														task.id,
													)}
								...
								<div
									class="mt-3 h-1.5 overflow-hidden rounded-full bg-gray-800"
								>
									<div
										class="h-full rounded-full bg-blue-500 transition-all duration-200"
										style="width: {Math.min(
											Math.max(task.progress * 100, 4),
											100,
										)}%"
									></div>
								</div>
			...
			{/if}
			<div
				class="audio-player-glass overflow-hidden rounded-2xl border shadow-2xl"
			>
						...
						>
							...
							{#if $playerStore.currentTrack}
								...
								>
									...
									<div class="min-w-0 flex-1">
										<h3
											class="truncate font-semibold text-white w-full"
										>
											{$playerStore.currentTrack
												.title}{!isSonglinkTrack(
												$playerStore.currentTrack,
											) &&
											asTrack($playerStore.currentTrack)
												.version
												? ` (${asTrack($playerStore.currentTrack).version})`
												: ""}
										</h3>
										{#if isSonglinkTrack($playerStore.currentTrack)}
											<!-- Display for SonglinkTrack -->
											<p
												class="truncate text-sm text-gray-400 w-full"
											>
												{$playerStore.currentTrack
													.artistName}
											</p>
										{:else}
											...
											<p
												class="truncate text-xs text-gray-500 w-full"
											>
												<a
							...
							>
								...
								>
									...
									<button
										...
										class="p-1.5 sm:p-2 text-gray-400 transition-colors hover:text-white disabled:opacity-50"
										disabled={$playerStore.queueIndex >=
								...
								<div class="hidden sm:flex items-center gap-2">
									<button
										...
										class="p-2 transition-colors hover:scale-110 active:scale-95 text-gray-400 hover:text-pink-400"
										aria-label={$playerStore.currentTrack &&
										libraryStore.isLiked(
									...
									>
										<Heart
											size={20}
											class={$playerStore.currentTrack &&
											libraryStore.isLiked(
												$playerStore.currentTrack.id,
											)
									...
									<div
										class="w-px h-4 bg-gray-700 mx-1"
									></div>
									<button
										onclick={toggleMute}
											...
											<li>
												...
												>
													...
													<div class="min-w-0 flex-1">
														...
														>
															{queuedTrack.title}{!isSonglinkTrack(
																queuedTrack,
															) &&
															asTrack(queuedTrack)
																.version
...
<style>
	.audio-player-glass {
		...
		backdrop-filter: blur(var(--perf-blur-high, 32px))
			saturate(var(--perf-saturate, 160%));
		-webkit-backdrop-filter: blur(var(--perf-blur-high, 32px))
			saturate(var(--perf-saturate, 160%));
		box-shadow:
	...
	.queue-panel {
		background: transparent;
		border-color: rgba(148, 163, 184, 0.2);
		backdrop-filter: blur(var(--perf-blur-medium, 28px))
			saturate(var(--perf-saturate, 160%));
